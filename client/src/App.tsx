...
function App() {
  ...
  return <div className="site">
    ...
    <main>
      ...
      <section id="menu" className="menu-section section-muted"><div className="container menu-heading"><div><p className="section-kicker">PARA EMPEZAR A ANTOJARTE</p><h2>Favoritos de la casa</h2></div></div><div className="container dish-grid">{dishes.map(([name,label,img])=><article className="dish" key={name}><div className="dish-image"><img src={img} alt={name}/><span>{label}</span></div><div className="dish-copy"><h3>{name}</h3><p>Preparado para disfrutarlo sin apuro.</p></div></article>)}</div><div className="menu-footer"><span>¿Ya se te antojó?</span><a className="btn btn-primary menu-btn" href="#reservas">Ven a probarlo <ChevronRight size={17}/></a></div></section>
